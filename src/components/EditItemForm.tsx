          ...
          <FormControl>
            <Input
              ...
              value={(() => {
                ...
                const cost = Number(form.watch('costPerUnit') || 0);
                return (quantity * cost).toFixed(2);
              })()}
            />
          </FormControl>
          <FormDescription>
            Calculated from Quantity x Cost Per Unit.
          </FormDescription>
        </FormItem>
...
export function EditItemForm({
  ...
  // Normalize stored location value (supports both legacy name values and id-based values).
  const initialLocationValue = React.useMemo(() => {
    const flattenedLocationOptions: Array<{ id: string; name: string }> = [];
    locations.forEach((location) => {
      flattenedLocationOptions.push({ id: location.id, name: location.name });
      location.children?.forEach((subLocation) => {
        flattenedLocationOptions.push({
          id: `${location.id}/${subLocation.id}`,
          name: `${location.name}/${subLocation.name}`,
        });
      });
    });

    const existingById = flattenedLocationOptions.find((option) => option.id === item.location);
    if (existingById) {
      return existingById.id;
    }

    const existingByName = flattenedLocationOptions.find((option) => option.name === item.location);
    if (existingByName) {
  ...
  const defaultValues = React.useMemo(() => {
    ...
    return {
      ...(item.expenseCode && !item.expenseTypeCode
        ? {
            expenseTypeCode: item.expenseCode.split(' - ')[0] || '',
            expenseTypeDescription: item.expenseCode.split(' - ').slice(1).join(' - ') || '',
          }
        : {}),
      name: item.name || '',
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="mt-6 flex justify-end space-x-2">
          ...
          <Button type="submit" disabled={false} className="min-h-11 px-4 text-sm">
