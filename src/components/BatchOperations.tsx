...
interface BatchOperationsProps {
  ...
  onReplaceItems: (items: InventoryItem[]) => void;
...
export default function BatchOperations({ allItems, selectedItems, onReplaceItems, onClearSelection, onItemsUpdated, onDelete }: BatchOperationsProps) {
  const [categories, setCategories] = React.useState<CategoryNode[]>([]);
  const [locations, setLocations] = React.useState<ItemWithSubcategories[]>([]);
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      setCabinets(savedCabinets ? JSON.parse(savedCabinets) : []);
    } catch {
      setCabinets([]);
    }
  }, []);
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      ...
      logger.info(
        'audit',
        'INVENTORY_ITEM_DELETED_BATCH',
        {
          count: selectedItems.length,
          itemIds: selectedItems.map((selectedItem) => selectedItem.id),
      ...
      logger.info(
        ...
        'BATCH_DELETE_COMPLETED',
        {
          ...
          performedBy: currentUser?.username || currentUser?.displayName || 'Unknown',
        },
        'BatchOperations'
      );
    } catch (error) {
  ...
  const handleBatchEdit = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        // Refresh inventory view state without leaving the current page.
        onItemsUpdated();
