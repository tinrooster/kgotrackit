...
5. Decide cloud strategy for logs + checkout activity (schema + RLS + payload size limits).
6. Custom report UX (save always discoverable); store custom report defs in `user_app_data` (`custom_report_definitions` JSONB) so they survive clean browsers once cloud sync runs for signed-in users.
7. **Backup / rollback:** implement **restore points** (named snapshots + list + restore) so daily file export is optional; add optional unattended file export where the platform allows; **System Logs** for snapshot/export/restore (see “Backup and Restore” above).
8. **Settings Backup tab + Lookup Lists UX:** condense settings snapshot copy; clarify full backup vs restore vs settings-only vs Import/Export; default Lookup Lists to **Categories** on open (see “Settings UX” above).
